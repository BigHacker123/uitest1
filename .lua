...
do
    ...
    function library:save_config(name, existscheck)
        ...
        if not table_find(options.ConfigSelected.values, name) then
            options.ConfigSelected:add_value(name)
        end
...
function library:create_settings_tab(menu, teleportallowed)
    ...
    settings_main:colorpicker({text = 'Accent', flag = 'ThemeAccent', default = library.themes.default.Accent, callback = function(color)
        library.theme.Accent = color
        library:update_theme()
    end})

    settings_main:toggle({text = 'Keybind Indicator', flag = 'KeybindIndicator', callback = function(bool)
    ...
    settings_config:dropdown({text = 'Config', flag = 'ConfigSelected'})
    settings_config:textbox({text = 'Config Name', flag = 'ConfigsInput'})

    settings_config:button({text = 'Create', confirm = true, callback = function()
        xpcall(function()
            library:save_config(flags.ConfigsInput, true)
            library:notification(("Successfully created config '%s'"):format(flags.ConfigsInput), 5, color3_new(0.35, 1, 0.35))
        end, function()
            library:notification(("Unable to create config '%s'"):format(flags.ConfigsInput), 5, color3_new(1, 0.35, 0.35))
        end)
    end})

    settings_config:button({text = 'Save', confirm = true, callback = function()
        xpcall(function()
            library:save_config(flags.ConfigSelected)
            library:notification(("Successfully saved config '%s'"):format(flags.ConfigSelected), 5, color3_new(0.35, 1, 0.35))
        end, function(err)
            library:notification(err or ("Unable to save config '%s'"):format(flags.ConfigSelected), 5, color3_new(1, 0.35, 0.35))
        end)
    end})

    settings_config:button({text = 'Load', confirm = true, callback = function()
        xpcall(function()
            library:load_config(flags.ConfigSelected)
            library:notification(("Successfully loaded config '%s'"):format(flags.ConfigSelected), 5, color3_new(0.35, 1, 0.35))
        end, function(err)
            library:notification(err or ("Unable to load config '%s'"):format(flags.ConfigSelected), 5, color3_new(1, 0.35, 0.35))
        end)
    end})
    ...
    if isfolder(library.cheatname .. '/' .. library.gamename .. '/configs') then
        for i,v in next, listfiles(library.cheatname .. '/' .. library.gamename .. '/configs') do
            ...
            if ext == '.txt' then
                options.ConfigSelected:add_value(v:split('\\')[#v:split('\\')]:sub(1,-#ext-1))
            end
        end
    end
